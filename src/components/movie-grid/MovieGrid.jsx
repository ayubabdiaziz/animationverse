        ...
        const getList = async () => {
            ...
            if (keyword === undefined || keyword.trim().length === 0) {
                switch(props.category) {
                    case category.movie:
                        response = await tmdbApi.getMoviesList(movieType.anime, {
                            genre: 16, // Animation genre ID
                        });
                        break;
                    default:
                        response = await tmdbApi.getTvList(tvType.anime, {
                            genre: 16, // Animation genre ID
                        });
                }
            } else {
                ...
                response.results = response.results.filter(item => item.genre_ids.includes(16));
            }
